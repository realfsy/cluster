...
//               : The number of elements must not be changed
//               (numel()==sz1*sz2*...) : Return the array with possibly new
...
template <typename T, typename SZ> class data_ptr {
public:
  using value_type = T;
  using size_type = SZ;

  data_ptr() : data_(nullptr), size_(0), capacity_(0), owner_(false)
  {
  }
  data_ptr(T *_data, SZ _sz)
      : data_(_data), size_(_sz), capacity_(_sz), owner_(false)
  {
  }
  ...
  data_ptr(data_ptr const &_other)
      : data_(_other.owner_ ? nullptr : _other.data_),
        ...
        capacity_(_other.owner_ ? 0 : _other.capacity_), owner_(_other.owner_)
  {
    if (owner_) {
      resize(_other.size_);
...
public:
  array_iterator() : arr_(nullptr), i_(0)
  {
  }
  array_iterator(array_iterator<T> const &other)
      : arr_(other.arr_), i_(other.i_)
  {
  }
  ~array_iterator()
  {
  ...
  array_iterator(T *_arr, typename T::size_type _i) : arr_(_arr), i_(_i)
  {
  }
...
class const_array_iterator
    : public std::iterator<std::random_access_iterator_tag,
                           typename T::value_type, typename T::size_type> {
public:
  const_array_iterator() : arr_(nullptr), i_(0)
  {
  }
...
template <int N> class numel {
public:
  template <typename SZ> static SZ compute(SZ _size[])
  {
    return _size[N - 1] * numel<N - 1>::compute(_size);
  }
};
template <> class numel<0> {
public:
  template <typename SZ> static SZ compute(SZ[])
  {
    return 1;
  }
};

// Compute the product for a set of numeric arguments: product<int32_T>(10, 20,
// 30, ...) => 10*20*30*...
template <typename SZ, typename First, typename... Rest> struct product_i {
  static SZ compute(First _f, Rest... _rest)
  {
    return _f * product_i<SZ, Rest...>::compute(_rest...);
  }
};
template <typename SZ, typename Last> struct product_i<SZ, Last> {
...
template <> class index_nd<0> {
public:
  template <typename SZ> static SZ compute(SZ[], SZ[])
  {
    return 0;
  }
};

template <bool Cond> struct match_dimensions {
};

template <> struct match_dimensions<true> {
  static void check()
  {
  }
};
...
// Base class for code::array. SZ is the type used for sizes (currently
// int32_t.) Overloading up to 10 dimensions (not using variadic templates to
// stay compatible with C++98.)
template <typename T, typename SZ, int N> class array_base {
public:
  using value_type = T;
  using size_type = SZ;
  ...
  template <typename... Dims> void set(T *_data, Dims... dims)
  {
...
private:
  ...
  template <SZ _i, typename Last> void set_size_i(Last l)
  {
    size_[_i] = l;
  }

public:
  template <typename... Dims> void set_size(Dims... dims)
  {
    coder::detail::match_dimensions<N == sizeof...(dims)>::check();
    set_size_i<0>(dims...);
  ...
  array_base<T, SZ, static_cast<SZ>(sizeof...(Dims))>
  reshape(Dims... dims) const
  {
  ...
  template <typename... Dims> SZ index(Dims... _dims) const
  {
  ...
  template <typename... Dims> T &at(Dims... _i)
  {
    coder::detail::match_dimensions<N == sizeof...(_i)>::check();
    return data_[index(_i...)];
  }

  template <typename... Dims> T const &at(Dims... _i) const
  {
    coder::detail::match_dimensions<N == sizeof...(_i)>::check();
    return data_[index(_i...)];
  }
...
template <typename T, int N> class array : public array_base<T, SizeType, N> {
private:
...
public:
  ...
  array(array<T, N> const &_other) : Base(_other)
  {
  }
  array(Base const &_other) : Base(_other)
  {
  }
  array(T *_data, SizeType const *_sz) : Base(_data, _sz)
  {
  }
};
...
template <typename T> class array<T, 2> : public array_base<T, SizeType, 2> {
private:
...
public:
  ...
  array(array<T, 2> const &_other) : Base(_other)
  {
  }
  array(Base const &_other) : Base(_other)
  {
...
// column vectors.
template <typename T> class array<T, 1> : public array_base<T, SizeType, 1> {
private:
...
public:
  ...
  array(array<T, 1> const &_other) : Base(_other)
  {
  }
  array(Base const &_other) : Base(_other)
  {
