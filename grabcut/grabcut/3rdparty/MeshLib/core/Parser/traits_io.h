	...
	for( M::MeshVertexIterator viter( pMesh ); !viter.end(); viter ++ )
	{
		...
		for( std::list<CToken*>::iterator iter = parser.tokens().begin() ; iter != parser.tokens().end(); ++ iter )
		{
		  ...
		  if( token->m_key == "k" )
		  {
			  ...
			  pV->target_k() = strutil::parseString<float>( line );	
		  }
		}
	}
};
	...
	for( M::MeshVertexIterator viter( pMesh ); !viter.end(); viter ++ )
	{
		...
		for( std::list<CToken*>::iterator iter = parser.tokens().begin() ; iter != parser.tokens().end(); ++ iter )
		{
		  ...
		  if( token->m_key == "z" )
		  {
			  CPoint2 uv;
			  token->m_value >> uv;
			  pV->z() = std::complex<float>( uv[0], uv[1] );
		  }
		}
	}
};
	...
	for( M::MeshVertexIterator viter( pMesh ); !viter.end(); viter ++ )
	{
		...
		for( std::list<CToken*>::iterator iter = parser.tokens().begin() ; iter != parser.tokens().end(); ++ iter )
		{
		  ...
		  if( token->m_key == "mu" )
		  {
			  CPoint2 uv;
			  token->m_value >> uv;
			  pV->mu() = std::complex<float>( uv[0], uv[1] );
		  }
		}
	}
};
...
void _read_edge_length( M * pMesh )
{
	for( M::MeshEdgeIterator eiter( pMesh ); !eiter.end(); eiter ++ )
	{
		...
		for( std::list<CToken*>::iterator iter = parser.tokens().begin() ; iter != parser.tokens().end(); ++ iter )
		{
		  ...
		  {
			 std::string line = strutil::trim( token->m_value, "()");
			 pE->length() = strutil::parseString<float>(line) ;		
		  }
		}
	}
};
	...
	for( M::MeshFaceIterator fiter( pMesh ); !fiter.end(); fiter ++ )
	{
		...
		for( std::list<CToken*>::iterator iter = parser.tokens().begin() ; iter != parser.tokens().end(); ++ iter )
		{
		  ...
		  if( token->m_key == "mu" )
		  {
			  CPoint2 uv;
			  token->m_value >> uv;
			  pF->mu() = std::complex<float>( uv[0], uv[1] );
		  }
		}
	}
};
