...
/*! set the pixel at (row, col)
	 ...
	 * \param red   the red color float value
	 * \param green the green color float value
	 * \param blue  the blue  color float value
	 */
void RgbImage::SetRgbPixelf( long row, long col, float red, float green, float blue )
{
	SetRgbPixelc( row, col, floatToUnsignedChar(red), 
							floatToUnsignedChar(green),
							floatToUnsignedChar(blue) );
}
/*! set the pixel at (row, col)
	 * \param row row position
	 * \param col column position
...
/*!	convert a float to unsigned char
	* \param x input float number
	* \return output unsigned char
	*/

unsigned char RgbImage::floatToUnsignedChar( float x )
{
	if ( x>=1.0 ) {
